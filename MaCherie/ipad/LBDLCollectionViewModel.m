...
-(id)initCollectionViewModelWithTexts:(NSArray*)texts andImages:(NSArray*)images {
    if (self = [super init]) {
        ...
        NSLog(@"Texts is: %d", (int)textsArray.count);
        imageArray = [[NSMutableArray alloc] initWithArray:images];
        NSLog(@"images is: %d", (int)images.count);
        theImagesAndTexts = [NSMutableArray array];
        customNumberOfResults = [NSNumber numberWithInt:(int)textsArray.count + (int)images.count];
...
-(void)randomizeTexts {
    ...
    for (int i = 0; i < (textsArray.count + imageArray.count); i++) {
