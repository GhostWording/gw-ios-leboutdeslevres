...
-(id)initWithFrame:(CGRect)frame collectionViewLayout:(UICollectionViewLayout *)layout {
    if (self = [super initWithFrame:frame collectionViewLayout:layout]) {
        ...
        activityIndicator.frame = CGRectMake(CGRectGetMidX(frame) - 80, CGRectGetMidY(frame) - 80, 160, 160);
...
-(id)initWithFrame:(CGRect)frame collectionViewLayout:(UICollectionViewLayout *)layout andTexts:(NSArray *)theTexts andImages:(NSArray *)theImages {
    if (self = [super initWithFrame:frame collectionViewLayout:layout]) {
        ...
        activityIndicator.frame = CGRectMake(CGRectGetMidX(frame) - 80, CGRectGetMidY(frame) - 80 - 34, 160, 160);
        [self addSubview:activityIndicator];
    }
...
-(void)updateData {
    ...
    [activityIndicator fadeInWithCompletion:^(BOOL completion){
        NSArray *tmpImageArray = [_imageAndTextDataSource updateImagesCollectionViewDataSource];
        NSArray *tmpTextArray = [_imageAndTextDataSource updateTextsCollectionViewDataSource];
        
        if (tmpImageArray != nil || tmpTextArray != nil) {
            [model setImages:tmpImageArray];
            [model setTexts:tmpTextArray];
            [model randomizeTexts];
        }
...
-(Text*)selectedText {
    if (self.selectedTextIndexPath != nil) {
        Text *theText = [[model randomImagesAndTexts] objectAtIndex:self.selectedTextIndexPath.row];
...
-(Image*)selectedImage {
    if (self.selectedImageIndexPath != nil) {
        Image *theImage = [[model randomImagesAndTexts] objectAtIndex:self.selectedImageIndexPath.row];
